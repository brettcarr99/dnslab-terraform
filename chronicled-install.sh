...
import argparse, datetime, errno, hashlib, hmac, json, os, re, syslog, time

# The default Chronicle version.
# Should be updated with each new Chronicle release
DEFAULT_CHRONICLE_VERSION = "chronicled-2.0.1692.0-1_naws"
...
def extract_region_from_fqdn(fqdn, domain):
  domains = [d.replace('.', '\.') for d in ["aws.a2z.com", "aws.a2z.com.cn", "c2s-a2z.ic.gov", "sc2s-a2z.sgov.gov", "a2z.hci.ic.gov", "a2z.adc-e.uk", domain]]
  domains = "(?:" + "|".join(domains) + ")"
  valid_control = re.compile("^(?:control\.prod\.)(?P<region>.+)(?:\.chronicle\.security\.)" + domains + "$")
  m = valid_control.match(fqdn)
  if m != None:
    return m.group('region')
  return ""


def get_dns_txt_record(fqdn):
  parser = re.compile("^(?:%s)(?:\.)(?:\s+)(?:[0-9]+)(?:\s+)IN(?:\s+)TXT(?:\s+)(?:\")(?P<txtvalue>.+)(?:\")$" % fqdn)
  try:
    result = subprocess.check_output(["dig", "-t", "TXT", "+noall", "+answer", fqdn])
  ...
  for line in lines:
    m = parser.match(line)
    if m != None:
      return m.group("txtvalue")
  return ""


def check_for_override(region, domain):
  txt_record = get_dns_txt_record("txt.prod.%s.chronicle.security.%s" % (region, domain))
  return extract_region_from_fqdn(txt_record, domain)
...
def get_region_and_creds():
  ...
  domain = get_metadata("/latest/meta-data/services/domain", token)
  override = check_for_override(region, domain)
  if override != "":
    region = override
  return region, creds["AccessKeyId"], creds["SecretAccessKey"], creds["Token"], domain
...
def download_rpm(ver):
  ...
  region, access_key, signing_key, session_token, domain = get_region_and_creds()
  ...
  if region == "us-iso-east-1":
    ...
    host = "control.prod.%s.chronicle.security.%s" % (region, domain)    
  else:
...
def install_dig():
  try:
    ...
    return 0 == subprocess.call(["yum", "-y", "install", "bind-utils"])
...
if __name__ == "__main__":
  try:
    ...
    if not install_dig():
      log_exit("dig is not available, and unable to install")
